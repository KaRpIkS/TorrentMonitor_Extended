...
class rXMLRPCRequest
{
	...
	public function rXMLRPCRequest($hostport, $cmds = null )
	{
		$hostport = explode(':', $hostport);
		$this->scgi_host = $hostport[0];
		$this->scgi_port = $hostport[1];
		if($cmds)
		{
		        if(is_array($cmds))
				foreach($cmds as $cmd)
	...
	public static function send($scgi_host, $scgi_port, $data )
	{
		$result = false;
		$contentlength = strlen($data);
		if($contentlength>0)
		{
			$socket = @fsockopen($scgi_host, $scgi_port, $errno, $errstr, 5);
			if($socket)
			{
	...
	public function run()
	{
		...
		if($this->makeCall())
		{
			$answer = self::send($this->scgi_host, $this->scgi_port, $this->content);
			if(!empty($answer))
			{
